...
class Recorder(object):
    ...
    def __init__(self, sensor_data_source: dict, gt_data_source: dict, record_config: dict):
        ...
        for gt_type_name, data_toggles in gt_record_config.items():
            # If any of data of this sensor is selected, set up a record buffer for it
            if self.must_record(data_toggles):
                self.gt_record_buffer[gt_type_name] = self.set_up_record_buffer(
                    data_toggles)
    ...
    def record_current_step(self):
        ...
        # Iterate over sensors
        for gt_type_name, record_buffer in self.gt_record_buffer.items():
            # Iterate over specific data
            for data_name in record_buffer:
                record_buffer[data_name].append(
                    self.gt_data_source[gt_type_name][data_name])
    ...
    def must_record(self, sensor_record_config):
        """ 
        ...
        Input:
            sensor_record_config: Dict of toggles specifying which data of a sensor has to be recorded.
        Output:
            True if must record this sensor. Otherwise, False. 
        """
        for toggle_value in sensor_record_config.values():
    ...
    def set_up_record_buffer(self, sensor_record_config):
        ...
        Input:
            sensor_record_config: Dict of toggles specifying which data of the sensor has to be recorded.
        Output:
        ...
        record_buffer = {}
        for data_name, toggle in sensor_record_config.items():
