    ...
    def render_vehicles_ids(self, vehicle_id_surface, list_actors, world_to_pixel, hero_actor, hero_transform):
        ...
        if self.show_actor_ids:
            ...
            for actor in list_actors:
                ...
                font_surface = self._header_font.render(
                    str(actor[0].id), True, color)
                rotated_font_surface = pygame.transform.rotate(
                    font_surface, angle)
        ...
        if self.show_info:
            ...
            for title, info in self._info_text.items():
                ...
                surface = self._header_font.render(
                    title, True, COLOR_ALUMINIUM_0).convert_alpha()
                ...
                for item in info:
                    ...
                    if isinstance(item, list):
                        ...
                        if isinstance(item[1], bool):
                            ...
                            pygame.draw.rect(
                                display, COLOR_ALUMINIUM_0, rect_border, 1)
                    ...
                    if item:  # At this point has to be a str.
                        surface = self._font_mono.render(
                            item, True, COLOR_ALUMINIUM_0).convert_alpha()
...
class TrafficLightSurfaces(object):
    ...
    def __init__(self):
        def make_surface(tl):
            ...
            if tl != 'h':
                ...
                pygame.draw.circle(surface, red if tl ==
                                   tls.Red else off, (hw, hw), int(0.4 * w))
                pygame.draw.circle(
    ...
    def rotozoom(self, angle, scale):
        ...
        for key, surface in self._original_surfaces.items():
            self.surfaces[key] = pygame.transform.rotozoom(
                surface, angle, scale)
...
class World(object):
    ...
    def start(self, hud, input_control):
        ...
        self.border_round_surface = pygame.Surface(
            self._hud.dim, pygame.SRCALPHA).convert()
    ...
    def _spawn_hero(self):
        ...
        while self.hero_actor is None:
            ...
            self.hero_actor = self.world.try_spawn_actor(
    ...
    def update_hud_info(self, clock):
        ...
        if self.hero_actor is not None:
            ...
            hero_mode_text = [
                ...
                'Hero Vehicle:  %14s' % get_actor_display_name(
                    self.hero_actor, truncate=14),
            ...
            hero_mode_text += lane_info_text
        else:
            hero_mode_text = ['Hero Mode:                OFF']

        self.server_fps = self.server_clock.get_fps()
        self.server_fps = 'inf' if self.server_fps == float(
            'inf') else round(self.server_fps)
        info_text = [
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        for v in list_v:
            ...
            pygame.draw.lines(surface, color, False, corners, int(
                math.ceil(4.0 * self.map_image.scale)))
    ...
    def render(self, display):
        ...
        if self.hero_actor is not None:
            ...
            center_offset = (
                abs(display.get_width() - self.surface_size) / 2 * scale_factor, 0)
...
class InputControl(object):
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    if event.key == K_q:
                        ...
                        if self._world.hero_actor is not None:
                            ...
                            self._hud.notification('Autopilot %s' % (
    ...
    def _parse_mouse(self):
        ...
        if pygame.mouse.get_pressed()[0]:
            ...
            self.mouse_offset[0] += (1.0 / self.wheel_offset) * \
                (x - self.mouse_pos[0])
            self.mouse_offset[1] += (1.0 / self.wheel_offset) * \
                (y - self.mouse_pos[1])
            self.mouse_pos = (x, y)
